...
function createZone(id, players, areaX, areaY) {
    let x = Math.floor(Math.random() * Math.floor(xBlockSize) + (areaX * Math.floor(xBlockSize)));
    let y = Math.floor(Math.random() * Math.floor(yBlockSize) + (areaY * Math.floor(yBlockSize)));
...
function createDrones(players, numberOfDrones) {
    ...
    for (let i = 0; i < players.length; i++) {
        for (let j = 0; j < numberOfDrones; j++) {
            ...
            drones.push({ droneID: i * numberOfDrones + j, owner: players[i].playerid, x: droneX, y: droneY });
        }
    }
...
function updateScores(scoresList, i, zones) {
    let filteredZones = zones.filter(z => z.owner == i);
